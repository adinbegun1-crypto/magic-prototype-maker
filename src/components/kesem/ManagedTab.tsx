          ...
          >
            <div className="flex justify-between items-start mb-3.5">
              ...
              <div className="text-right ml-2 shrink-0">
                ...
                <p className="text-[10px] text-muted-foreground mt-0.5">projected / yr</p>
              </div>
            </div>

            <div className="mb-1.5" style={{ height: 70 }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={fund.projectedChartData} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
                  <defs>
            ...
            <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-3">
              Simulated projected growth · demo data
            </p>
...
function FundDetail({ fund, onBack }: { fund: Fund; onBack: () => void }) {
  ...
  const chartData = useMemo(
    () => buildProjectedGrowthSeries(fund.name, low, parsedAmount),
    [fund.name, low, parsedAmount],
  );
    ...
    return (
      <div className="animate-fade-up text-center py-8 px-4">
        ...
        <p className="text-sm text-muted-foreground mb-7 leading-relaxed">
          ₪{parsedAmount.toLocaleString()} is now in your{" "}
      ...
      <div className="bg-card rounded-2xl p-5 shadow-sm">
        ...
        <div className="mb-4">
          <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-2">
            Simulated projected growth on ₪{parsedAmount.toLocaleString()} · 12 months
          </p>
        ...
        <div className="bg-background rounded-xl p-4 mb-5 space-y-2.5">
          {([
            ["Projected annual return", fund.expectedReturn, fund.color],
            ["Annual fee (0.5%)", `₪${(parsedAmount * 0.005).toFixed(0)}`, "hsl(var(--foreground))"],
        ...
        >
          Invest ₪{parsedAmount.toLocaleString()} in {fund.name}
        </button>
      </div>
    </div>
  );
