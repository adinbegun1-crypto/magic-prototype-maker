    ...
    <div className="animate-fade-up">
      <div className="bg-card rounded-2xl overflow-hidden shadow-sm">
        ...
        {transactions.map((tx, i) => (
          <div key={tx.id} className="px-5 py-4 flex justify-between items-center" style={{ borderBottom: i < transactions.length - 1 ? "1px solid #F4F4F0" : "none" }}>
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-semibold" style={{ background: tx.type === "credit" ? "#D8F3DC" : "#FFF0F0", color: tx.type === "credit" ? "#2D6A4F" : "#e05252" }}>
                {tx.type === "credit" ? "↓" : "↑"}
              </div>
              <div>
            ...
            <p className="text-sm font-semibold" style={{ color: tx.type === "credit" ? "#2D6A4F" : "#1a1a1a" }}>
